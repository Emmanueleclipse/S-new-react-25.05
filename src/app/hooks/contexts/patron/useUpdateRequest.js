...
const UpdateRequestContext = React.createContext();

export const UpdateRequestProvider = (props) => {
  ...
  const mapDataToUpdate = (formInfo) => {
    ...
    return {
      ...
      items: items.map(mapItemToUpdate),
    };
  };
  const resetInfo = () => {
  ...
  return <UpdateRequestContext.Provider value={value} {...props} />;
};

export default () => useContext(UpdateRequestContext);
